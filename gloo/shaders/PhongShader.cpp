...
void PhongShader::SetLightSource(const LightComponent& component) const {
  ...
  auto dir_light_ptr = static_cast<DirectionalLight*>(light_ptr);
  // Upload the ray direction. Send world-space direction to match.
  glm::vec3 dir_world = dir_light_ptr->GetDirection();
  if (glm::length(dir_world) > 0.0f)
    dir_world = glm::normalize(dir_world);
  SetUniform("directional_light.enabled", true);
  SetUniform("directional_light.direction", dir_world);
  SetUniform("directional_light.diffuse",  dir_light_ptr->GetDiffuseColor());
  SetUniform("directional_light.specular", dir_light_ptr->GetSpecularColor());
  } else {
